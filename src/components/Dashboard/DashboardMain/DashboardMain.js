...
class DashboardMain extends Component {
  ...
  handleCoverLetter() {
    setTimeout(() => {
      this.setState((prevStat, props) => ({ iscomingSoonShowed: !prevStat.iscomingSoonShowed }))
    }, 2000);
    this.setState((prevStat, props) => ({ iscomingSoonShowed: !prevStat.iscomingSoonShowed }))
  }
  // Show deleted toast
          ...
          <div className="dashboardNavigaitionList">
            <ul>
              ...
              <li><Link onClick={() => this.handleCoverLetter()} className="dashboardNavItem">Cover Letter</Link>
                <AnimatePresence>
                  {this.state.iscomingSoonShowed && <motion.a initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="statusSoon">coming soon</motion.a>}
                </AnimatePresence>
              </li>
            </ul>
          </div>
